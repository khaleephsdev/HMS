...
$query="insert into  room_registration (roomno,seater,feespm,foodstatus,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
				...
				<div class="row">
					<div class="col-md-12">
						...
						<div class="row">
							...
							<?php
$id=$_SESSION['login'];
				$stmt=$mysqli->prepare("SELECT emailid FROM room_registration WHERE emailid=? ");
...
<input type="radio" value="1" name="foodstatus"> With Food( N20,000.00 Per Month Extra)
</div>
</div>	
...
<?php	
$id=$_SESSION['id'];
	$ret="select * from user_registration where id=?";
